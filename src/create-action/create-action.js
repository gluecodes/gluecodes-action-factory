...
const inputSchema = require('./create-action-schema');
...
const _bindStepToStepResults = ({
  ...
  return (props) => { // eslint-disable-line consistent-return
    const stepResultOrPromise = code(props);

    if (stepResultOrPromise instanceof Promise) {
      return stepResultOrPromise
        .then(stepResult => storeStepResult({ stepName: name, stepResult }));
    }

    const stepResult = stepResultOrPromise;
...
const _mergeStepResultRecursively = ({
  ...
  Object.keys(propsToIterateThrough).forEach((propName) => {
    ...
    const valueToBeSet = isItNestedObjectIteration && value !== null ? value[propName] : value;
    ...
    if (valueToBeSet !== null && (!isValueLiteralObject || hasNestedObjectNoPropsDefined)) {
